...
public class NeuralNetwork {
    ...
    public static void CreateTrainingData() {
		...
		try
		{
			BufferedReader br = new BufferedReader(new FileReader("C:\\Users\\Hp\\Downloads\\input.csv"));  
			...
			int count = 0;
			BufferedReader br1 = new BufferedReader(new FileReader("C:\\Users\\Hp\\Downloads\\input.csv"));  
			while ((line = br1.readLine()) != null)      
			{  
				String[] input = line.split(splitBy);  
				float[] inputs = new float[input.length-1];
				for(int q = 0; q < input.length - 1 ; q++)
				{
					inputs[q] = Float.parseFloat(input[q]);
				}
				float[] expectedOutput = new float[] {Float.parseFloat(input[3])};
				tDataSet[count] = new TrainingData(inputs, expectedOutput);
				count = count+1;
			}  
		}
		catch (IOException e)  
    ...
    public static void train(int training_iterations,float learning_rate) {
    	for(int i = 0; i < training_iterations; i++) {
			float i1 = i;
			float progress = (i1/training_iterations)*100;
    		for(int j = 0; j < tDataSet.length; j++) {
    			forward(tDataSet[j].data);
    			backward(learning_rate,tDataSet[j]);
    		}
			System.out.println("======Training in Progess " + progress + " % completed========");
    	}
    }
}